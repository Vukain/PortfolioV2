...
export const useGsapProjectTriggers = (infoRef: InfoRef, index: number, numberOfProjects: number, currentProject: number, setCurrentProject: React.Dispatch<React.SetStateAction<number>>) => {

    const infoTL: React.MutableRefObject<null | gsap.core.Timeline> = useRef(null);
    const elements = useRef<{ slidingText: null[] | HTMLElement[], slidingCapsules: null[] | HTMLElement[] }>({ slidingText: [], slidingCapsules: [] });
    ...
    useEffect(() => {
        ...
        if (currentProject === index && slidingText.length > 0) {
            ...
            animationTimeout = setTimeout(() => {
                infoTL.current!
                    .to(slidingText, { xPercent: '0', yPercent: '0', duration: duration, stagger: motionReducerSwitch((isDesktop ? .09 : .2)), ease: 'sine.inOut', opacity: 1 })
                    .to(slidingCapsules, { delay: motionReducerSwitch(-(duration - .6), -duration), yPercent: '0', duration: motionReducerSwitch(.3, duration), stagger: motionReducerSwitch(.1), opacity: 1, ease: 'sine.inOut' })
            }, 1300);
        };
